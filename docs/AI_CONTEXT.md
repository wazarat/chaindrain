...
│       ├── src/lib/exposure/{predicates,aadapt_map,similarity}.ts ← Phase 6 — 24 ROOT_CAUSE_PREDICATES + AADAPT maps + Jaccard/cosine/ensemble math (shared by seeder + tests)
│       ├── src/lib/exposure/{predicates,similarity}.test.ts   ← Phase 6 — 24 predicate cases + Jaccard/cosine/ensemble math tests
│       ├── scripts/lib/{demo_rand,demo_fixtures,demo_rand.test}.ts ← Phase 6 — deterministic RNG + fixtures + RNG determinism tests
│       ├── scripts/seed_exposure_demo.ts                      ← Phase 6 — Layer 1 seeder, 5 batched UPSERTs via `jsonb_to_recordset`
│       ├── scripts/seed_incidents_demo.ts                     ← Phase 6 — Layer 2 incident ledger seeder (356 rows, victim selection via ROOT_CAUSE_PREDICATES)
│       ├── scripts/seed_similarity.ts                         ← Phase 6 — Layer 3 similarity seeder (Method A/B/C ensemble, top-25 per source)
│       ├── src/app/exposure/{page,[entity_id]/page,incidents/page,incidents/[incident_id]/page}.tsx ← Phase 6 — four routes
│       ├── src/app/api/exposure/{twins,peers}/[entity_id]/route.ts ← Phase 6 — JSON endpoints
│       ├── src/components/{demo-chip,demo-banner,exposure-kpi-cards,exposure-table,exposure-profile,exposure-panels,incidents-table}.tsx ← Phase 6 UI
...
| `chaindrain.governance_fingerprint` | **772** (Phase 6, PM #12) | Backfilled by Layer 1 seeder; all rows `data_confidence='DEMO'`. |
| `chaindrain.reputation_signal` | **772** (Phase 6, PM #12) | Backfilled by Layer 1 seeder + Layer 2 incident seeder (323 rows have `last_known_incident_date`). |
| `chaindrain.incident` | **356** (Phase 6, PM #12) | Incident Ledger seeded by Layer 2 (`seed_incidents_demo.ts`); deterministic per `seedFromEntityId` × root_cause spec. Triangular date density peaked at 2024-06. |
| `chaindrain.similarity_pair` | **19,300** (Phase 6, PM #12) | 772 × 25 (top-25 per source). Method A weighted Jaccard, Method B incident-overlap (`min(1, B/5)`), Method C SHA-256 64-dim fake-embedding cosine. Ensemble weights 0.3 / 0.4 / 0.3. Avg ensemble 0.25–0.50 across the universe; RealT top-5 = BlackRock BUIDL, Kelp DAO, Backed Finance, Lift Dollar (USDL), Protocol-Native Treasury Agents. |
...
### Phase 6 — Exposure Graph (4th tab) — COMPLETE locally as of 2026-05-16 PM #12
...
**Universe:** the canonical 772 entities from `chaindrain.mvp_master_dedup`. Every Phase 6 query and seeder targets this view.

**Shipped in PM #12 (this session):**
1. **Layer 1 perf rewrite** — `scripts/seed_exposure_demo.ts` now builds in-memory payloads and executes one `INSERT/UPDATE … FROM jsonb_to_recordset(...)` per target table (5 batches total: identity, contract_fingerprint, dependency_fingerprint, governance_fingerprint, reputation_signal). End-to-end runtime dropped from 226s+ (killed) to 726ms. Confidence gating preserved via `CASE WHEN existing_confidence IN ('HIGH','MEDIUM','INFERRED') THEN existing ELSE EXCLUDED.x END`.
2. **Layer 2 — `scripts/seed_incidents_demo.ts`** — generates 356 incidents (one `INSERT … FROM jsonb_to_recordset(...)`), victims sampled from `ROOT_CAUSE_PREDICATES[rc]`-eligible entities, triangular date density centred on 2024-06, AADAPT tactic/technique chips. Reputation backfill: 323 entities get `last_known_incident_date = MAX(event_date)` via a single bulk UPDATE. Runtime ~300ms.
3. **Layer 3 — `scripts/seed_similarity.ts`** — 772 sources × 771 targets scored in JS (~340ms), top-25 per source persisted in ~3s. Method C uses two domain-separated SHA-256 hashes (`:lo`/`:hi`) concatenated to fill the 64-dim vector — earlier single-hash version produced `NaN` for the upper 32 buckets. Math factored into `src/lib/exposure/similarity.ts` so seeder + tests share one implementation.
4. **Query layer** — `listExposureEntities`, `getExposureEntity`, `getThreatHistory`, `getPeerIncidents`, `getDependencyTwins`, `listIncidents`, `getIncidentById`, `getExposureKpis`. All `*Cached` siblings with three new tag constants: `CACHE_TAG_EXPOSURE_LAYER1`, `CACHE_TAG_EXPOSURE_INCIDENTS`, `CACHE_TAG_EXPOSURE_SIMILARITY`.
5. **API routes** — `/api/exposure/twins/[entity_id]` (zod-validated `limit`) + `/api/exposure/peers/[entity_id]` (computes `matchingRootCauses(predicateEntity)` server-side, then returns `{ matched_root_causes, groups }`).
6. **UI** — `<DemoChip />`, `<DemoBanner />`, `<ExposureKpiCards />`, `<ExposureTable />`, `<ExposureProfile />` (Identity/Contract/Dependency/Governance/Reputation expander), `<ThreatHistoryPanel />`, `<PeerIncidentsPanel />`, `<DependencyTwinsPanel />`, `<IncidentsTable />`.
7. **Pages** — `/exposure` (KPI strip + table), `/exposure/[entity_id]` (profile + three panels with `#threat-history` / `#peer-incidents` / `#dependency-twins` anchors), `/exposure/incidents` (paginated ledger), `/exposure/incidents/[incident_id]` (full detail with AADAPT chips + evidence section).
8. **Site header** — `active` union widened to include `"exposure"`; nav adds Exposure Graph link with Hydra-Teal `Preview` pill (`bg-teal-700/15 text-teal-700`).
9. **Methodology** — new `#exposure-graph` anchor section covering Methods A/B/C, weights 0.30/0.40/0.30, worked example (RealT → BUIDL/Kelp/Backed/USDL), and a "what is synthetic today" callout listing incident ledger + DEMO/INFERRED Layer 1 fields + Method C embeddings.
10. **Tests** — `predicates.test.ts` (24 predicate cases + total-over-minimal-input guard + matchingRootCauses determinism), `similarity.test.ts` (Jaccard / weighted methodA / fakeEmbed unit norm + determinism / cosineClamped / methodBNormalize / ensembleScore), `demo_rand.test.ts` (mulberry32, seedFromEntityId, weighted, intInRange, triangularDate, logNormalLoss). **84 vitest cases total, all green.**
11. **CI gates** — `pnpm typecheck` ✓, `pnpm lint` ✓, `pnpm build` ✓ (Next 16.2.6 turbopack, all four `/exposure*` routes registered), `pnpm test` ✓ 84/84.

**Remaining for v0.1.0 ship:**
- Commit + push to `main` with `wazarat <wazarat@outlook.com>` authorship.
- Smoke `chaindrain-mvp.vercel.app/exposure` and `/exposure/<RealT entity_id>` once the deploy lands. Verify the three panels populate, the DemoBanner shows, the Preview pill is in the nav.
